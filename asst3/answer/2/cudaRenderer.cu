...
__global__ void kernelRenderPixel() {
    ...
    int pieceX = pieceIndex % numPieceWidth; // x 对应 width
    int pieceY = pieceIndex / numPieceWidth;

    int startX = pieceX * widthEachPiece;
    int startY = pieceY * heightEachPiece;
    // 当前块的范围
    float boxL = startX * invWidth;
    float boxR = (startX + widthEachPiece) * invWidth;
    float boxB = startY * invHeight;
    float boxT = (startY + heightEachPiece) * invHeight;
    ...
    for (int index = 0; index < cuConstRendererParams.numCircles; index++) {
        ...
        for (int pixelY = startY; pixelY < startY + heightEachPiece; pixelY++) {
            float4* imgPtr = (float4*)(&cuConstRendererParams.imageData[4 * (pixelY * imageWidth + startX)]);
            for (int pixelX = startX; pixelX < startX + widthEachPiece; pixelX++) {
                float2 pixelCenterNorm = make_float2(invWidth * (static_cast<float>(pixelX) + 0.5f),
                                                     invHeight * (static_cast<float>(pixelY) + 0.5f));
                shadePixel(index, pixelCenterNorm, p, imgPtr);
                imgPtr++;
...
CudaRenderer::render() {
    ...
    // 256 threads per block is a healthy number
    dim3 blockDim(4, 1);
    dim3 gridDim((numPieceHeight * numPieceWidth + blockDim.x - 1) / blockDim.x);
