...
int main() {
    ...
    const unsigned int N = 20 * 1000 * 3000; // 20 M element vectors (~80 MB)
    ...
    //
    // Run the serial thread implementation. Repeat three times for robust
    // timing.
    //
    double minSerialThread = 1e30;
    for (int i = 0; i < 3; ++i) {
        double startTime =CycleTimer::currentSeconds();
        saxpyThread(N, scale, arrayX, arrayY, resultSerial);
        double endTime = CycleTimer::currentSeconds();
        minSerialThread = std::min(minSerialThread, endTime - startTime);
    }

    printf("[saxpy serial thread]:\t[%.3f] ms\t[%.3f] GB/s\t[%.3f] GFLOPS\n",
            minSerialThread * 1000,
            toBW(TOTAL_BYTES, minSerialThread),
            toGFLOPS(TOTAL_FLOPS, minSerialThread));
